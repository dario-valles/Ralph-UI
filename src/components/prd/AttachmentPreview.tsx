...
function AttachmentChip({ attachment, onRemove, disabled }: AttachmentChipProps) {
  ...
  return (
    <div
      className={cn(
        'group relative flex items-center gap-2 rounded-lg border bg-card p-1.5 pr-2',
...
function AttachmentImage({ attachment }: AttachmentImageProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          'absolute inset-x-0 bottom-0 flex items-center gap-1 px-2 py-1',
          'bg-black/60 text-white text-xs opacity-0 hover:opacity-100 transition-opacity'
        )}
      >
        <ImageIcon className="h-3 w-3" />
        <span className="truncate">{attachment.filename || 'Image'}</span>
        <span className="text-white/70 ml-auto">{formatFileSize(attachment.size)}</span>
      </div>
    </a>
  )
}
