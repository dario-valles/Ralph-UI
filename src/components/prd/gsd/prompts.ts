...
export function detectVagueAnswer(answer: string): { isVague: boolean; reason?: string } {
  ...
  const vaguePatterns = [
    ...
    {
      pattern: /make (it |things )?easier/i,
      reason: 'Easier how? What specific pain point does it address?',
    },
    {
      pattern: /^(users|people|everyone)$/i,
      reason: "Can you be more specific about who? What's their job or situation?",
    },
    { pattern: /better (than|experience)/i, reason: 'Better in what measurable way?' },
    {
      pattern: /simple|easy|intuitive/i,
      reason: 'What specific design choices would make it simple?',
    },
