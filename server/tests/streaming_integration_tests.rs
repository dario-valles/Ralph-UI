...
mod streaming_integration_tests {
    ...
    async fn test_agent_manager_emits_subagent_events() {
        ...
        let event1: ralph_ui_lib::agents::SubagentEvent = rx.recv().await.expect("Should receive Spawned event");
        ...
        let event2: ralph_ui_lib::agents::SubagentEvent = rx.recv().await.expect("Should receive Progress event");
        ...
        let event3: ralph_ui_lib::agents::SubagentEvent = rx.recv().await.expect("Should receive Completed event");
    ...
    async fn test_agent_manager_maintains_tree_state() {
        ...
        let tree = manager.get_subagent_tree(agent_id).expect("Tree should exist");
